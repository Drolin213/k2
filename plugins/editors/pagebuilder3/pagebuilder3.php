	...
	private function canIUsePB($id, $option, $view = '')
	{
        return in_array($id, $this->supportedArea)
			...
			//					|| ($id === 'content' && $option === 'com_easyblog')
                    ...
                    || (in_array($id, array('art_fulltext', 'cat_description')) && $option === 'com_cck' && $view === 'form')
                )
            );
	}
	...
	public function onDisplay($name, $content, $width, $height, $col, $row, $buttons = true, $id = null, $asset = null, $author = null, $params = array())
	{
        ...
        if (!$this->canIUsePB($id, $option, $app->input->get('view', ''))) {
	        ...
	        if (JPluginHelper::isEnabled('captcha', 'recaptcha'))
	        {
		        ...
		        $recaptcha = json_decode($recaptcha->params);

		        // Make public key and private key are available.
		        if (empty($recaptcha) || empty($recaptcha->public_key) || empty($recaptcha->private_key))
		        {
		        	unset($recaptcha);
		        }
	        }

	        if (JPluginHelper::isEnabled('captcha', 'recaptcha_invisible'))
	        {
		        $invisible_recaptcha = JPluginHelper::getPlugin('captcha', 'recaptcha_invisible');
		        $invisible_recaptcha = json_decode($invisible_recaptcha->params);

		        // Make public key and private key are available.
		        if (empty($invisible_recaptcha)
			        || empty($invisible_recaptcha->public_key) || empty($invisible_recaptcha->private_key))
		        {
			        unset($invisible_recaptcha);
		        }
	        }
			...
			<script type="text/javascript">
				...
				window.pagefly_data = window.pb2_editor_data = window.pb_editor_data = ' . json_encode(array(
					...
					'invisible_recaptcha' => isset($invisible_recaptcha) ? $invisible_recaptcha : null
				)) . ';
			</script>
